...
export default function AdminTestimonials({
  ...
  const { data, actions, ui } = useListPage({
    endpoint: adminEndpoints.testimonials.list,
  });
  ...
  const deleteModal = useModal<{ id: number | string; client_name?: string; deleteApi: string }>();
  ...
  return (
    <div className="admin-testimonials">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => createModal.open({ createApi: adminEndpoints.testimonials.create })}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                      <Actions
                        ...
                        onEdit={() => editModal.open({
                          fetchApi: adminEndpoints.testimonials.show(item.id),
                          updateApi: adminEndpoints.testimonials.update(item.id)
                        })}
                        showView={false}
                        showDelete={false}
                        additionalActions={[
                          {
                            ...
                            action: () => deleteModal.open({
                              ...
                              deleteApi: adminEndpoints.testimonials.delete(item.id)
                            }),
                            icon: "trash",
                          },
                        ]}
      ...
      {deleteModal.isOpen && deleteModal.data && (
        <ConfirmModal
          ...
          title="Xác nhận xóa"
          message={`Bạn có chắc nhận muốn xóa đánh giá từ "${deleteModal.data.client_name}"? Hành động này không thể hoàn tác.`}
